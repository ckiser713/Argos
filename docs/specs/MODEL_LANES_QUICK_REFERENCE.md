...
CORTEX_LANE_SUPER_READER_MODEL_PATH=/models/nemotron-4m.gguf
CORTEX_LANE_SUPER_READER_BACKEND=llama_cpp

# Coder (vLLM)
CORTEX_LANE_CODER_URL=http://localhost:8000/v1
CORTEX_LANE_CODER_MODEL=Qwen3-Coder-30B-1M

# Fast-RAG (vLLM)
CORTEX_LANE_FAST_RAG_URL=http://localhost:8000/v1
CORTEX_LANE_FAST_RAG_MODEL=MegaBeam-Mistral-7B-512k

# Governance (llama.cpp)
CORTEX_LANE_GOVERNANCE_URL=http://localhost:8080/v1
CORTEX_LANE_GOVERNANCE_MODEL=Granite-4.x-Long-Context
CORTEX_LANE_GOVERNANCE_MODEL_PATH=/models/granite-4m.gguf
...
1. **Check for lane-specific config** (`CORTEX_LANE_{LANE}_URL` and `CORTEX_LANE_{LANE}_MODEL`)
2. **Fall back to default lane** (`CORTEX_LLM_DEFAULT_LANE`, usually `orchestrator`)
3. **Use default config** (`CORTEX_LLM_BASE_URL` and `CORTEX_LLM_MODEL`)
